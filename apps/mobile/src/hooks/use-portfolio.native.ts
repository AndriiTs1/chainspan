...
import type { Portfolio, PortfolioAssetFailure, TokenBalanceInput } from '@chainspan/web3';
...
// Same policy as apps/web/hooks/use-portfolio.ts.
...
export function usePortfolio(): UsePortfolioResult {
  ...
  const erc20BalanceQuery = useReadContracts({
    ...
    query: {
      enabled: Boolean(address) && chainIsSupported && registryTokens.length > 0,
      staleTime: BALANCE_STALE_TIME_MS,
      refetchOnWindowFocus: true,
      refetchOnReconnect: true,
    },
  ...
  const stillLoading =
    nativeBalanceQuery.isLoading || (erc20QueryActive && erc20BalanceQuery.isLoading);
  ...
  const { portfolio, failedAssets } = buildPortfolio(address, chainId, balanceInputs);
